...
## HDD
* データを磁気情報で表現、磁気ディスク(フラッタ)に記憶するストレージデバイス
* データはバイト単位ではなく、セクタ単位(512B or 4KB)で読み書きする
* データ転送の流れ
    1. デバイスドライバが、出たの読み書きに必要な情報(セクタ番号、セクタの数、アクセス種など)をHDDに渡す
    2. スイングアームを動かし、フラッタを回転させ、アクセスするセクタ上に磁気ヘッドの位置を合わせる
    3. データの読み書き
* スイングアームの動作とフラッタの回転がボトルネックとなる
* 連続するセクタにアクセスする場合に比べ、連続していないセクタにアクセスするのはとろい
    * プログラムでは、データは連続する領域に確保し、そこへのアクセスは複数回に分けるのではなく一まとめにすべき

## ブロックデバイス層の機能
* I/Oスケジューラ
    * 以下の処理をしてからデバイスドライバにIO要求を行うことで、スループットを向上させる
    * マージ
        * 複数の連続するセクタへのIO要求を1つにまとめる
    * ソート
        * 複数の不連続なセクタへのIO要求をセクタ番号順に整列する
* 先読み
    * ストレージデバイスのある領域にアクセスした際、それに続く領域のデータを先読みしておく
        * 空間的局所性を利用
    * 推測通りのアクセスが来なかったら、単に先読みデータを捨てる

## SSD
* データへのアクセスに機械的動作がなく、電気的動作だけで済む
* よって、HDDより高速
    * とくにランダムアクセスのスループットが高い